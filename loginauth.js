...
// Function to login a user
  function login_usr()
  {
    var log_username_parse = document.getElementById("login_usr").value.toLowerCase().trim(); //this is the entered value in DOM
    var log_usr = document.getElementById("login_email").value.trim(); //email entered by user
    var docRef = db.collection("Users").doc(log_username_parse);
    var log_psswd = document.getElementById("login_psswd").value.trim(); //password entered by user
      ...
      if ((doc.exists))
      {
          ...
          { 
              // console.log("done");
              localStorage.setItem("account_login_username_NOTES", log_username_parse);
...
// create a message when account does not exist
      ...
      else if((doc.data() == undefined))
      {
        // doc.data() will be undefined in this case
        var error = document.getElementById("error");
          error.innerHTML = "Your Account hasn't been registered with Memorandum, please SIGN-UP by clicking  on the sign-up button";
      }
      else 
      {
        // 
          var error = document.getElementById("error");
          error.innerHTML = "Please Try Logging-in once again!";  
...
else if (localStorage.getItem("account_login_username_NOTES") === null) {
  form = document.getElementById("form");
  document.getElementById("re_enter").style.display = "none";
  reg = document.getElementById("register");
  reg.style.transform = "translateY(750px)";
